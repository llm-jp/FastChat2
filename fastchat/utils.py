...
function() {
    ...
    const msg = "利用者は、本サービスを利用する前に以下の規約に同意する必要があります。\\n\\n- 本サービスは LLM-jp の参加者のみが利用できます。本サービスのアドレス、ユーザ名、パスワードを LLM-jp の参加者以外に知らせないでください。\\n- 本サービスは利用者個人を特定できる情報を収集しません。ただし、アクセスログや端末情報など個人を特定できない情報については、運用上必要な範囲で収集する場合があります。\\n- 利用者が本サービスへ入力した情報（以下、「入力情報」）および本サービスから生成・表示される出力（以下、「出力」）は、LLM-jp での研究開発に利用する場合があります。\\n- 入力情報および出力は、Creative Commons Attribution（CC-BY）またはそれに類するライセンスのもと一般に公開する可能性があります。\\n- 出力の再利用および再配布を禁止します。\\n- 出力は LLM-jp および関連組織の公式な見解や意見を代表するものではありません。\\n- 利用者が本サービスを利用することにより、または利用できなかったことにより生じたいかなる損害についても、LLM-jp は一切の責任を負いません。\\n- アクセスが集中した場合やシステム障害、その他の理由により、本サービスは予告なく停止することがあります。\\n- 利用者による不正行為または不正行為の疑いがあると判断した場合、事前の通知なしに本サービスの利用を制限することがあります。";
